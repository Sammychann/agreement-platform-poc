...
async def submit_form(
    ...
    if customer_signature:
        cust_path = sig_dir / f"customer_{customer_signature.filename}"
    ...
    if msd_signature:
        msd_path = sig_dir / f"msd_{msd_signature.filename}"
        with msd_path.open("wb") as buffer:
            shutil.copyfileobj(msd_signature.file, buffer)
        signatures_info["msd_signature"] = str(msd_path)

    # We use a trick here: we overwrite the actual form_data object to be used by logger 
    # but logger expects AgreementFormData, which is what parsed_data is.
    # However, excel logger generates its own entry_id, let's update excel_logger to use passed ID or we return logger's ID.
    # Wait, the prompt said excel_logger returns entry_id. We'll use the ID from excel_logger and rename sig dir.
    
    actual_entry_id = excel_logger.log_entry(parsed_data, signatures_info)
    
    # Rename sig dir to actual entry id
    new_sig_dir = SIGNATURES_DIR / actual_entry_id
    if sig_dir.exists() and sig_dir != new_sig_dir:
        sig_dir.rename(new_sig_dir)
        # Update paths
        if "customer_signature" in signatures_info:
            signatures_info["customer_signature"] = signatures_info["customer_signature"].replace(entry_id, actual_entry_id)
        if "msd_signature" in signatures_info:
            signatures_info["msd_signature"] = signatures_info["msd_signature"].replace(entry_id, actual_entry_id)
            
        # We should ideally re-save these in the logger, but let's just keep it simple.
        # Actually logger saved them with the old paths. So let's generate ID first.
...
async def edit_agreement(agreement_id: str, updated_fields: Dict[str, Any]):
    ...
    # Extract entry_id from filename: {entry_id}_{type}_{id}.docx
    filename = path.name
    ...
    return {"preview_url": f"/api/generate/preview/{new_id}"}
